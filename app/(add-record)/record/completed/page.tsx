...
export default function CompletedPage() {
  ...
  useEffect(() => {
    ...
    if (!raw || status !== 'completed') { router.replace('/record/add'); return }
    setBook(JSON.parse(raw))
  }, [router])

  const canSave = !!startDate && !!endDate && rating > 0

  async function handleSave() {
    if (!book || !canSave) return
    setSaving(true)
    const { error } = await saveBookRecord({
      book, status: 'completed', start_date: startDate, end_date: endDate, rating, review: review || undefined,
    })
    setSaving(false)
    if (!error) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F5F0E8] pb-[52px]">
      ...
      <div className="h-[44px] px-5 flex items-center justify-between border-b border-[#D4CAC2] bg-[#F5F0E8]">
        ...
        <span className="text-[17px] font-semibold text-[#3D2C24]">완독</span>
        <div className="w-6" />
      </div>
      ...
      <div className="px-5 py-6 space-y-6">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-[14px] font-semibold text-[#3D2C24]">독서 기간 <span className="text-[#B94040]">*</span></label>
          <div className="grid grid-cols-2 gap-3">
            <input
              type="date"
              value={startDate}
              onChange={(e) => setStartDate(e.target.value)}
              className="h-[48px] rounded-xl border border-[#D4CAC2] bg-white px-4 text-[14px] text-[#3D2C24] outline-none focus:border-[#4A7C59]"
            />
            <input
              type="date"
              value={endDate}
        ...
        {/* 평점 */}
        <div className="flex flex-col gap-2">
          <label className="text-[14px] font-semibold text-[#3D2C24]">평점 <span className="text-[#B94040]">*</span></label>
          <StarRating rating={rating} onRatingChange={setRating} size={32} />
        </div>

        {/* 감상평 */}
        <div className="flex flex-col gap-2">
          <label className="text-[14px] font-semibold text-[#3D2C24]">감상평</label>
          <textarea
            value={review}
      ...
      <button
        ...
        className="fixed bottom-0 left-0 right-0 h-[52px] bg-[#4A7C59] text-white text-[16px] font-semibold transition-opacity"
