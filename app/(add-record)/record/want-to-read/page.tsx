...
export default function WantToReadPage() {
  ...
  useEffect(() => {
    ...
    if (!raw || status !== 'want-to-read') { router.replace('/record/add'); return }
    setBook(JSON.parse(raw))
  }, [router])

  const canSave = expectation > 0

  async function handleSave() {
    if (!book || !canSave) return
    setSaving(true)
    const { error } = await saveBookRecord({
      book, status: 'want-to-read', expectation, review: memo || undefined,
    })
    setSaving(false)
    if (!error) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F5F0E8] pb-[52px]">
      ...
      <div className="h-[44px] px-5 flex items-center justify-between border-b border-[#D4CAC2] bg-[#F5F0E8]">
        ...
        <span className="text-[17px] font-semibold text-[#3D2C24]">읽고 싶은</span>
        <div className="w-6" />
      </div>
      ...
      <div className="px-5 py-6 space-y-6">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-[14px] font-semibold text-[#3D2C24]">기대치 <span className="text-[#B94040]">*</span></label>
          <StarRating rating={expectation} onRatingChange={setExpectation} size={32} />
        </div>

        {/* 기대평 */}
        <div className="flex flex-col gap-2">
          <label className="text-[14px] font-semibold text-[#3D2C24]">기대평</label>
          <textarea
            value={memo}
            onChange={(e) => setMemo(e.target.value)}
            placeholder="이 책에 기대하는 점을 적어보세요"
