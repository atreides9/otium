...
export default function PausedPage() {
  ...
  useEffect(() => {
    ...
    if (!raw || status !== 'paused') { router.replace('/record/add'); return }
  ...
  async function handleSave() {
    ...
    setSaving(true)
    const { error } = await saveBookRecord({ book, status: 'paused', start_date: startDate, progress })
    setSaving(false)
    if (!error) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F5F0E8] pb-[52px]">
      ...
      <div className="h-[44px] px-5 flex items-center justify-between border-b border-[#D4CAC2] bg-[#F5F0E8]">
        ...
        <span className="text-[17px] font-semibold text-[#3D2C24]">잠시 중단</span>
        <div className="w-6" />
      </div>
      ...
      <div className="px-5 py-6 space-y-6">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <label className="text-[14px] font-semibold text-[#3D2C24]">현재 진도</label>
            <span className="text-[20px] font-bold text-[#C47D2E]">{progress}%</span>
          </div>
          <input
            type="range"
            min={0}
            ...
            className="w-full accent-[#C47D2E]"
          />
        </div>
      </div>
