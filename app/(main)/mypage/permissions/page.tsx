...
export default function PermissionsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0EDE8] px-5 py-6 space-y-4">
      <h1 className="text-xl font-bold text-[#1A1A1A]">앱 권한 관리</h1>
      ...
      >
        ...
        {PERMISSIONS.map((perm, i) => (
          <div
            ...
            style={i < PERMISSIONS.length - 1 ? { borderBottom: '1px solid #F0EDE8' } : undefined}
          >
      ...
      >
        ...
        <button
          ...
          style={{ border: '1px solid #E5E1DC', color: '#1A1A1A', backgroundColor: 'transparent' }}
        >
          기기 설정 열기
        </button>
      </div>
        ...
        <ul className="space-y-2.5">
          {[
            ...
            <li key={i} className="flex items-start gap-2">
              <span className="mt-1 flex-shrink-0 w-1.5 h-1.5 rounded-full" style={{ backgroundColor: '#C4973A' }} />
              <span className="text-xs leading-relaxed" style={{ color: '#666666' }}>
                {text}
              </span>
            </li>
          ))}
